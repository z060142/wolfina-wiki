...
- quick_query: Fetch data from the wiki or files and get a concise summary back
  in one call. The query agent searches and reads internally, so you don't have
  to page through raw results yourself.
  USE FOR: looking up existing facts, page content, file contents, history.
  DO NOT USE FOR: analysis, reasoning, similarity judgements, or evaluating
  content you already have in context — do that reasoning yourself.
  Required: query (string) — be specific about what data you want.
